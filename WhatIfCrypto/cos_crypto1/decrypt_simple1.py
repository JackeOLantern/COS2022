...
class Encryptor(object):
    ...
    def decrypt(self, plaintext):
        ctrBytes = plaintext[0:16]
        print("ctr bytes = "+str(ctrBytes))
        ctr = int.from_bytes(ctrBytes, 'big')
        print("ctr = "+str(ctr))
        decrypted = b''
        #16 first bytes is the key (ctr)
        for i in range(16, len(plaintext), 16):
            decrypted += bytes(self.xor(plaintext[i:i+16], ctr.to_bytes(16, 'big')))
            ctr += 1
        # get rid of trailing b'\0' from padding and line feed
        print("last ", decrypted[-1])
        while decrypted[-1] == 0:
            decrypted = decrypted[0:-1]
            #print("- cut next is ", decrypted[-1])
...
if __name__ == '__main__':
    ...
    ciphertext = Encryptor(passphrase).decrypt(plaintext)
    if args.output_file:
        ...
        print('Your file after decryption is', ciphertext.hex())
